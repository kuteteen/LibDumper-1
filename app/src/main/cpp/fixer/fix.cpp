...
static void
_fix_relative_rebase(char *buffer, size_t bufSize, uint64_t imageBase, Elf_Shdr_Type *g_shdr) {
...
template<typename Elf_Ehdr_Type, typename Elf_Shdr_Type, typename Elf_Phdr_Type,
        ...
        bool isElf32>
static void _regen_section_header(const Elf_Ehdr_Type *pehdr, const char *buffer, size_t len,
    ...
    for (int i = 0; i < nDynSyms; i++) {
        ...
        if (type > STT_FILE) {
            ...
            if (sym->st_value == 0) {
                ...
                uint32_t flag = _get_mem_flag<Elf_Phdr_Type, Elf_Addr_Type>(phdr, ph_num,
                                                                            sym->st_value);
                if (flag & PF_X) {
                    newType = STT_FUNC;
                }
...
static void _fix_elf(char *buffer, size_t flen, FILE *fw, uint64_t ptrbase) {
    ...
    _fix_relative_rebase<Elf_Shdr_Type, Elf_Addr_Type, Elf_Rel_Type, isElf32>(buffer, flen, ptrbase,
                                                                              g_shdr);
...
int fix_so(const char *openPath, const char *outPutPath, uint64_t ptrbase) {
    ...
    if (isElf32) {
        _fix_elf<Elf32_Ehdr, Elf32_Shdr, Elf32_Phdr, Elf32_Word, Elf32_Addr, Elf32_Sym, Elf32_Dyn, Elf32_Rel, true>(
                buffer, flen, fw, ptrbase);
    } else {
        _fix_elf<Elf64_Ehdr, Elf64_Shdr, Elf64_Phdr, Elf64_Word, Elf64_Addr, Elf64_Sym, Elf64_Dyn, Elf64_Rela, false>(
                buffer, flen, fw, ptrbase);
    }
